...
import { X, ChevronDown, ChevronUp, Link2, Loader2, CheckCircle2, AlertCircle } from 'lucide-react';
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setImportError(data.error || 'Errore sconosciuto');
        return;
      }

      // Popola formData con i campi ricevuti
      const location = data.fullAddress || data.location || '';
      const newFormData: any = {
        ...formData,
        title: data.title || formData.title,
        price: data.price || formData.price,
        sqm: data.sqm || formData.sqm,
        location,
        link: data.link || importUrl.trim(),
      };

      if (data.floor != null)       newFormData.floor = data.floor.toString();
      if (data.condition)           newFormData.condition = data.condition;
      if (data.heating)             newFormData.heating = data.heating;
      if (data.energyClass)         newFormData.energyClass = data.energyClass;
      if (data.kwh != null)         newFormData.kwh = Math.round(data.kwh).toString();
      if (data.condoFees != null)   newFormData.condoFees = data.condoFees.toString();

      setFormData(newFormData);

      // Aggiorna i display degli input formattati
      if (data.price) {
        const formatted = data.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
        setPriceInput(formatted);
      }
      if (data.sqm) setSqmInput(data.sqm.toString());

      // Apri sezione dettagli se ci sono campi importati
      if (data.floor != null || data.condition || data.heating || data.energyClass || data.kwh != null) {
        setShowDetails(true);
      }
      ...
      setImportStatus('error');
      setImportError('Errore di rete — controlla la connessione');
    } finally {
      setImporting(false);
    }
  };
        ...
        <>
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Sezione import da URL */}
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                <div className="flex items-center gap-2">
                  <Link2 className="w-3.5 h-3.5 text-slate-400" />
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                    Importa da immobiliare.it
                  </span>
                  <span className="text-[9px] text-slate-300 ml-auto">Opzionale</span>
                </div>
                <div className="flex gap-2">
                  <input
                    type="url"
                    placeholder="https://www.immobiliare.it/annunci/..."
                    className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                    value={importUrl}
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-slate-900 text-white rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-slate-700 transition-all disabled:opacity-40 disabled:cursor-default flex items-center gap-1.5 shrink-0"
                  >
                    {importing
                ...
                {importStatus === 'success' && (
                  <div className="flex items-center gap-2 text-emerald-600">
                    ...
                    <span className="text-[10px] font-medium">Campi importati — verifica e completa</span>
                  </div>
                )}
                {importStatus === 'error' && (
