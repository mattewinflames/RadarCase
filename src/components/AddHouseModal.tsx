...
import { X, ChevronDown, ChevronUp, Radar } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { House } from '../types';
import { ImportDraft } from '../lib/importListing';
...
interface Props {
  ...
  onClose: () => void;
  appMode: 'buy' | 'rent';
  /** Dati letti da un annuncio tramite bookmarklet. Riempiono il form all'apertura. */
...
export default function AddHouseModal({ onAdd, isOpen, onClose, appMode, prefill }: Props) {
  ...
  // Coordinate arrivate dall'annuncio (solo Immobiliare le espone).
  // Se presenti, l'immobile nasce gia' posizionato e l'app salta la geocodifica.
  ...
  // Riporta il modulo allo stato iniziale. Usata dopo il salvataggio e quando
  // si apre un inserimento manuale dopo un'importazione, per non trascinarsi dietro
  // i dati (soprattutto le coordinate) dell'annuncio precedente.
  const resetForm = () => {
    setShowErrors(false);
  ...
  // Riempie il form con i dati letti dall'annuncio.
  // `prefill` non e' fra le dipendenze oltre all'apertura: la bozza viene creata
  // una volta sola e non deve poter sovrascrivere le correzioni fatte a mano.
  useEffect(() => {
    if (!isOpen) return;

    // Apertura manuale ("Aggiungi al Radar"). Se l'apertura precedente era
    // un'importazione chiusa senza salvare, si riparte da un modulo pulito:
    // altrimenti la nuova casa erediterebbe le coordinate dell'annuncio importato.
    if (!prefill) {
      if (importSource || importedCoords) resetForm();
      return;
    }
    ...
    setFormData(prev => ({
      ...
      sqm: prefill.sqm ?? prev.sqm,
      location: prefill.location ?? prev.location,
      link: prefill.link ?? prev.link,
      yearBuilt: prefill.yearBuilt ?? '',
      floor: prefill.floor ?? '',
      condition: prefill.condition ?? '',
      heating: prefill.heating ?? '',
      kwh: prefill.kwh ?? '',
      energyClass: prefill.energyClass ?? '',
      condoFees: prefill.condoFees ?? '',
    }));

    if (prefill.price) setPriceInput(formatItalianNumber(String(prefill.price)));
    if (prefill.sqm) setSqmInput(String(prefill.sqm));

    setImportedCoords(
      typeof prefill.lat === 'number' && typeof prefill.lng === 'number'
        ? { lat: prefill.lat, lng: prefill.lng }
        : null
    );
    setImportSource(prefill.source ?? null);
    setShowDetails(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Coordinate gia' note dall'annuncio: l'immobile nasce posizionato,
    // cosi' l'app non spende una chiamata di geocodifica per trovarlo.
    if (importedCoords) {
      finalFormData.lat = importedCoords.lat;
      finalFormData.lng = importedCoords.lng;
    }
        ...
        <>
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              {importSource && (
                <div className="flex items-start gap-2.5 rounded-xl border border-indigo-100 bg-indigo-50 px-3.5 py-3">
                  <Radar className="text-indigo-600 shrink-0 mt-0.5" size={16} />
                  <div className="text-xs text-slate-600 leading-relaxed">
                    Dati letti da <strong className="text-slate-800">{importSource}</strong>. Controllali e completa
                    voto e note.
                    {importedCoords
                      ? ' La posizione sulla mappa è già inclusa.'
                      : ' La posizione verrà cercata dall\'indirizzo dopo il salvataggio.'}
                  </div>
                </div>
              )}
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Valutazione</label>
