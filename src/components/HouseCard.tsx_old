...
import { ExternalLink, MapPin, Trash2, Navigation, Edit2, Check, X as CloseX, AlertCircle, RefreshCw } from 'lucide-react';
...
const HouseCard: React.FC<HouseCardProps> = ({ house, settings, onDelete, onToggleVisited, onUpdate, onRetryGeocoding, isSelected, onSelect }) => {
  ...
  const handleSaveCommute = () => {
    onUpdate(house.id, {
      commute: {
        daughter: { distance: tempDistances.daughter },
        work: { distance: tempDistances.work }
      }
    });
      ...
      <div className="p-5">
        <div className="flex justify-between items-start mb-3" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-1">
            <button 
              onClick={() => setIsEditingCommute(!isEditingCommute)}
              className="text-slate-300 hover:text-blue-500 transition-colors p-1"
              title="Modifica distanze"
            >
        ...
        <div className="grid grid-cols-2 gap-3 mb-6" onClick={e => e.stopPropagation()}>
          {currentDestinations.daughter.label && (
            <div 
              ...
              className={`p-3 bg-slate-50 rounded-xl border transition-all cursor-pointer ${
                isEditingCommute ? 'border-blue-300 ring-2 ring-blue-50' : 'border-slate-100 hover:border-blue-200 hover:bg-slate-100/50'
              }`}
            >
              ...
              <div className="flex items-center gap-1">
                {isEditingCommute ? (
                  <input 
                    autoFocus
                    type="text"
                    className="bg-white border border-blue-200 rounded px-1.5 py-0.5 text-xs w-full focus:outline-none"
                    value={tempDistances.daughter}
                    onChange={e => setTempDistances({...tempDistances, daughter: e.target.value})}
                    onKeyDown={e => e.key === 'Enter' && handleSaveCommute()}
                    placeholder="km"
                  />
                ) : (
                  <>
              ...
              <div className="flex items-center gap-1">
                {isEditingCommute ? (
                  <input 
                    type="text"
                    className="bg-white border border-blue-200 rounded px-1.5 py-0.5 text-xs w-full focus:outline-none"
                    value={tempDistances.work}
                    onChange={e => setTempDistances({...tempDistances, work: e.target.value})}
                    onKeyDown={e => e.key === 'Enter' && handleSaveCommute()}
                    placeholder="km"
                  />
                ) : (
        ...
        {isEditingCommute && (
          <div className="flex gap-2 mb-4 animate-in fade-in slide-in-from-top-2">
            <button 
              ...
              className="flex-1 bg-blue-600 text-white rounded-lg py-1.5 text-[10px] font-bold uppercase tracking-widest hover:bg-blue-700 transition-colors flex items-center justify-center gap-1"
            >
              <Check className="w-3 h-3" /> Salva KM
            </button>
            <button 
              onClick={() => setIsEditingCommute(false)}
              className="px-3 bg-slate-100 text-slate-500 rounded-lg py-1.5 text-[10px] font-bold uppercase tracking-widest hover:bg-slate-200 transition-colors flex items-center justify-center"
            >
