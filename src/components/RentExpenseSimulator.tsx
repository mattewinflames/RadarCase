...
export default function RentExpenseSimulator({ houses, selectedHouseId, onHouseSelect }: Props) {
  ...
  const [showLivingCosts, setShowLivingCosts] = useState(false);
  ...
  const calcs = useMemo(() => {
    ...
    const monthlyRecurring = rent;
    const yearlyRecurring = monthlyRecurring * 12;
    const yearlyLiving = (rent + condoFees) * 12;
                  ...
                  <div className="text-left md:text-right">
                    ...
                    <div className="bg-amber-600 px-6 py-3 rounded-2xl shadow-xl shadow-amber-900/40">
                      <div className="flex items-center justify-between gap-4 mb-2">
                        <p className="text-[9px] text-amber-100 uppercase font-bold tracking-widest whitespace-nowrap">
                          {showLivingCosts ? 'Spese di vita annue' : 'Canone annuo'}
                        </p>
                        <button
                          onClick={() => setShowLivingCosts(!showLivingCosts)}
                          className={`shrink-0 w-8 h-4 rounded-full relative transition-colors ${showLivingCosts ? 'bg-white/30' : 'bg-white/10'}`}
                          title="Includi spese condominio"
                        >
                          <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${showLivingCosts ? 'left-4' : 'left-0.5'}`} />
                        </button>
                      </div>
                      <p className="text-2xl font-bold text-white font-mono leading-none">
                        € {(showLivingCosts ? calcs.yearlyLiving : calcs.yearlyRecurring).toLocaleString('it-IT')}
                      </p>
                      <p className="text-[9px] text-amber-200 mt-1 font-mono">
                        € {(showLivingCosts ? (rent + condoFees) : rent).toLocaleString('it-IT')} / mese
                        {showLivingCosts && condoFees > 0 && <span className="ml-1 opacity-70">(+€{condoFees} cond.)</span>}
                      </p>
                    </div>
                  </div>
                </div>
