...
const HouseCard: React.FC<HouseCardProps> = ({ house, settings, onDelete, onToggleVisited, onUpdate, onRetryGeocoding, isSelected, onSelect }) => {
  ...
  const [detailsForm, setDetailsForm] = useState({
    ...
    buildingFloors: house.buildingFloors?.toString() || '',
  ...
  const handleGenerateQuestions = async () => {
    ...
    try {
      ...
      const details = [
        ...
        house.buildingFloors ? `Piani edificio: ${house.buildingFloors}` : null,
        ...
        <div className="p-5">
          ...
          {(house.condition || house.energyClass || house.yearBuilt || house.heating) && (
            <div className="flex flex-wrap gap-1.5 mb-4 mt-2" onClick={e => e.stopPropagation()}>
              ...
              {house.energyClass && (
                <span className="px-2 py-0.5 bg-green-50 text-green-600 text-[9px] font-bold uppercase rounded-full">
              ...
              {!loadingQuestions && questions.length > 0 && (
                <div className="p-4 border-t border-slate-100">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 text-[10px] font-bold uppercase tracking-widest rounded-xl transition-colors"
                  >
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1.5">Piani edificio</label>
                    <input
                      type="number"
                      placeholder="es. 4"
                      min="1"
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm"
                      value={detailsForm.buildingFloors}
                      onChange={e => setDetailsForm({...detailsForm, buildingFloors: e.target.value})}
                    />
                  </div>
                  <div>
              ...
              <div className="p-4 border-t border-slate-100">
                <button
                  onClick={() => {
                    ...
                    if (detailsForm.buildingFloors) updates.buildingFloors = Number(detailsForm.buildingFloors);
