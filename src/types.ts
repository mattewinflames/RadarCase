...
export interface House {
  ...
  // Stato visita
  visitStatus?: 'da_visitare' | 'appuntamento' | 'visitata';
  appointmentAt?: number;    // epoch ms – data/ora appuntamento fissato
  visitedAt?: number;        // epoch ms – data in cui è avvenuta la visita
  // Note post-visita e valutazione qualitativa AI
  visitNotes?: string;       // testo libero scritto dopo la visita
  visitScore?: number;       // 1-10 assegnato da Gemini
  visitScoreReason?: string; // motivazione sintetica del punteggio
  visitScoreAt?: number;     // epoch ms dell'ultimo calcolo
}

/**
 * Risolve lo stato visita di un immobile, gestendo la migrazione da `visited: boolean`.
 * Usato sia in HouseCard sia in App per i contatori.
 */
export function resolveVisitStatus(
  house: Pick<House, 'visitStatus' | 'visited'>
): 'da_visitare' | 'appuntamento' | 'visitata' {
  if (house.visitStatus) return house.visitStatus;
  return house.visited ? 'visitata' : 'da_visitare';
}
